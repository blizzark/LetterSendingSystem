...
namespace LetterSendingSystem.Connect
{
    ...
    internal static class LetterRepository
    {
        ...
        {
            using HttpResponseMessage? response = await Request.Get($"{Request.hostName}{Routes.LETTERS}{userId}/{page}").ConfigureAwait(false);
        ...
        public static async Task<List<Letter>?> GetListUserHistory(int userId, int page)
        {
            ...
            using HttpResponseMessage? response = await Request.Get($"{Request.hostName}{Routes.HISTORY}{userId}/{page}").ConfigureAwait(false);
        ...
        {
            using HttpResponseMessage? response = await Request.Post($"{Request.hostName}{Routes.SEND_LETTER}", letter).ConfigureAwait(false);
        }
    }
}
